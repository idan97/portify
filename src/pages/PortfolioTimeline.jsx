...
export default function PortfolioTimeline() {
  ...
  // Raw loaded data kept in state so the timeline regenerates when the USD
  // rate arrives/changes.
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Timeline is generated in the rate-aware effect above.
    } catch (error) {
  ...
  const generateTimelineData = (
    ...
    const sortedManualValues = [...manualAssetValues].sort(
      (a, b) => parseISO(a.date) - parseISO(b.date),
    );

    const firstHoldingDate =
      sortedHoldings.length > 0 ? parseISO(sortedHoldings[0].date) : null;
    const firstManualDate =
      sortedManualValues.length > 0
        ? parseISO(sortedManualValues[0].date)
        : null;

    let firstDate;
    if (firstHoldingDate && firstManualDate) {
      firstDate =
        firstHoldingDate < firstManualDate ? firstHoldingDate : firstManualDate;
    } else {
    ...
    months.forEach((month) => {
      ...
      // Process Holdings
      const holdingsUpToMonth = sortedHoldings.filter(
        (h) => parseISO(h.date) <= monthEnd,
      );
      const latestHoldings = new Map();
      holdingsUpToMonth.forEach((h) => latestHoldings.set(h.instrument_id, h));

      assetClasses.forEach((ac) => {
        const classInstruments = instruments.filter(
      ...
      const manualValuesUpToMonth = sortedManualValues.filter(
        (v) => parseISO(v.date) <= monthEnd,
      );
      const latestManualValues = new Map();
      manualValuesUpToMonth.forEach((v) =>
              ...
              <div>
                ...
                <p
                  className={`text-2xl font-bold ${
                    totalStats.annualGrowthRate >= 0
                      ? "text-emerald-600"
                      : "text-red-500"
                  }`}
                >
                  {totalStats.annualGrowthRate >= 0 ? "+" : ""}
                  {totalStats.annualGrowthRate.toFixed(1)}%
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-purple-600">
                  {totalStats.bestAsset
                    ? `${totalStats.bestAsset.performance >= 0 ? "+" : ""}${totalStats.bestAsset.performance.toFixed(1)}%`
                    : "N/A"}
                </p>
            ...
            <div className="h-96 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-slate-600">
                  Add some instrument values to see your portfolio timeline.
                </p>
              </div>
            </div>
          ) : (
            <div className="h-96">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={filteredTimelineData}>
                  ...
                  />
                  {Object.keys(filteredTimelineData[0] || {})
                    ...
                    .map((assetName) => (
                      <Line
                        ...
                        type="monotone"
                        dataKey={assetName}
                        stroke={getAssetColor(assetName)}
                        strokeWidth={2}
                        dot={{ r: 3 }}
                        name={assetName}
                      />
